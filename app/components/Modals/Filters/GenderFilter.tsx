...
interface GenderFilterProps {
  value?: GenderType[];
  onChange?: (levels: GenderType[]) => void;
}
...
export const GenderFilter = ({
  ...
  const [selectedLevels, setSelectedLevels] = useState<Set<GenderType>>(
  ...
  useEffect(() => {
    if (selectAllCheckboxRef.current) {
      const isIndeterminate =
        selectedLevels.size > 0 && selectedLevels.size < genderOptions.length;
      selectAllCheckboxRef.current.indeterminate = isIndeterminate;
    }
  }, [selectedLevels]);

  const handleChange = (level: GenderType, isSelected: boolean) => {
  ...
  const handleSelectAll = (isSelected: boolean) => {
    if (isSelected) {
      const allLevels = genderOptions.map((opt) => opt.value);
  ...
  return (
    <div className="mt-3 flex w-full flex-col items-end gap-3">
      <Checkbox
        ...
        radius="sm"
        color="secondary"
        isSelected={selectedLevels.size === genderOptions.length}
        isIndeterminate={
          selectedLevels.size > 0 && selectedLevels.size < genderOptions.length
        }
